...
- 占内存大小：字符常量只占2个字节; **字符串常量占若干个字节(40+2*n)** [String的大小](https://blog.csdn.net/mafei6827/article/details/87076371)
...
- **byte、short和char类型运算时会自动转为int，因此结果也是int类型**
...
```
Integer i = new Integer(100);
int j = 100；//比较时会自动装箱和拆箱
System.out.print(i == j); //true
```

3、非new生成的Integer变量和new Integer()生成的变量比较时，结果为false。（因为**非new生成的Integer变量指向的是java常量池中的对象，而new Integer()生成的变量指向堆中新建的对象**，两者在内存中的地址不同）

```
Integer i = new Integer(100);//来自于堆
Integer j = 100;//来自于常量池
System.out.print(i == j); //false
```
...
主要作用是完成对类对象的初始化工作。可以执行。因为一个类即使没有声明构造方法也会有默认的不带参数的构造方法。但是当自己写了一个有参构造的时候程序就不会帮我们自动创建这个无参构造，此时如果我们不自己手写无参构造函数的话就会报错。
...
父类的**私有属性和构造方法**并不能被继承，所以 Constructor 也就不能被 override（重写）,但是可以 overload（重载）,所以你可以看到一个类中有多个构造函数的情况。
...
- 都不能够被实例化，接口的的实现类和抽象类的子类都只有在实现了接口和抽象类的方法后才能够被实例化
...
1. Java抽象类可以有构造函数吗？
    可以有，抽象类可以声明并定义构造函数。因为你不可以创建抽象类的实例，所以构造函数只能通过构造函数链调用（Java中构造函数链指的是从其他构造函数调用一个构造函数），例如，当你创建具体的实现类。现在一些面试官问，如果你不能对抽象类实例化那么构造函数的作用是什么？好吧，它可以用来初始化抽象类内部声明的通用变量，并被各种实现使用。另外，即使你没有提供任何构造函数，编译器将为抽象类添加默认的无参数的构造函数，没有的话你的子类将无法编译，因**为在任何构造函数中的第一条语句隐式调用super()，Java中默认超类的构造函数。**
...
-  情况1：类没有覆盖 equals() 方法。则通过 equals() 比较该类的两个对象时，比较的是引用对象的内存地址是否相同
...
2. 在异常语句之前使用了System.exit(int)已退出程序。 exit是带参函数 ；若该语句在异常语句之后，finally会执行
3. 程序所在的线程死亡。
4. 关闭CPU。
...
- finalize 是 Object 类的一个protected方法，垃圾回收器准备释放内存的时候，会先调用finalize()。任何对象的finalize 方法只会被调用一次。JVM 不保证此方法总被调用
...
- InvocationHandler的核心就是动态代理处理程序：invoke方法，invoke会返回我们我们想要的结果。
...
- LinkedList并没有实现这个接口，推荐使用Iterator和foreach()

## HashTable

- 使用 synchronized 来保证线程安全
- hashtable不可以有null值；
- Hashtable中数组默认大小是11，扩容的时候hashtable是乘以2加1。
- Hashtable 的底层数据结构都是采用 **数组+链表** 的形式

## HashMap

- Hashmap是不安全的；
- HashMap则可以有空值；
- Hashmap默认大小是16，扩容hashmap是乘以2，负载因子0.75.
- 底层数据结构：1.7之前是**数组+链表**， JDK1.8 以后是**数组+链表+红黑树**，当链表长度大于8并且数组长度超过64时（数组不超过64时先扩容数组），将链表转化为红黑树，以减少搜索时间。
...
- hash值与右移16位的hash做异或运算，得到经过扰动函数后的hash(**高16bit不变，低16bit和高16bit做了一个异或，目的是减少碰撞**)
...
- **PUT**
    ...
    计算hash值找到元素在数组中的位置，如果需要扩容则是rehash之后再找位置；hash值在HashMap中不存在则插入元素，存在则通过equal比较是否为同一个值，**是同一个值则更新value**，不是同一个值则使用尾插法插入链表尾部。如果链表元素超过8则转换为红黑树

- **GET**

    计算 K 的 hash 值从而获取该键值所在链表的数组下标；遍历链表，equals()方法查找相同 Node 链表中 K 值对应的 V 值。

### 为什么用红黑树而不用二叉搜索树

二叉搜索树会退化成线性结构，这和链表就没有区别了。引入红黑树就是为了查找数据快，解决链表查询深度的问题，红黑树属于平衡二叉树，但是为了保持“平衡”是需要付出代价的，但是该代价所损耗的资源要比遍历线性链表要少，所以当长度大于8的时候，会使用红黑树，如果链表长度很短的话，根本不需要引入红黑树，引入反而会慢。

## ConcurrentHashMap

- **底层数据结构：** JDK1.7的 ConcurrentHashMap 底层采用 **分段的数组+链表** 实现，JDK1.8 采用数组+链表/红黑树。数组是 HashMap 的主体，链表则是主要为了解决哈希冲突而存在的；
- **实现线程安全的方式（重要）：** ① **在JDK1.7的时候，ConcurrentHashMap（分段锁）** 对整个桶数组进行了分割分段(Segment)，每一把锁只锁容器其中一部分数据，多线程访问容器里不同数据段的数据，就不会存在锁竞争，提高并发访问率。 **到了 JDK1.8 的时候已经摒弃了Segment的概念，而是直接用 Node 数组+链表+红黑树的数据结构来实现，并发控制使用 synchronized 和 CAS 来操作。整个看起来就像是优化过且线程安全的 HashMap，虽然在JDK1.8中还能看到 Segment 的数据结构，但是已经简化了属性，只是为了兼容旧版本； **

### ConcurrentHashMap工作原理

- **PUT**

    **如果没有 hash 冲突就直接 CAS 无锁插入**；如果需要数组扩容，就先进行扩容；**如果存在 hash 冲突，就加锁来保证线程安全**，两种情况：一种是链表形式就直接遍历到尾端插入，一种是红黑树就按照红黑树结构插入；如果该链表的数量大于阀值 8，就要先转换成红黑树的结构。

- **GET**

    get操作第一步取key的hash值，如果是首节点符合，利用unsafe类和**volatile**修饰符来获取最新的值，然后返回。如果不是首节点那就开始遍历链表和红黑树使用equal()方法查找元素并返回，获取不到则返回null。
