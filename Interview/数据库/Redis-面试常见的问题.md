...
数据存在内存中可持久化，使用单线程操作的一种key-value数据库。
...
- 每个 哨兵以 每秒钟 一次的频率，向它所知的 主、从服务器 以及其他 Sentinel 实例 发送一个 PING 命令。如果一段时间未收到回复就会被标记为主观下线
...
**将数据均匀的分散到各个节点中，并且尽量的在加减节点时能使受影响的数据最少。**

一致 Hash 算法是将所有的哈希值构成了一个环，其范围在 `0 ~ 2^32-1`。之后将各个服务器节点散列到这个环上，可以用服务器节点的 IP、hostname 这样的唯一性字段作为 Key 进行 `hash(key)`。然数据可以根据 hash值选择离他最近的一个节点进行存储。这样带来的好处就是节点无需将所有数据进行持久化。而且当节点增加或减少的时候只会影响该节点内的数据。
